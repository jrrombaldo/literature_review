...
class utils():
    ...
    def diff(ris_a, ris_b):
        studies_a = set(utils.list_titles_from_ris(ris_a))
        studies_b = set(utils.list_titles_from_ris(ris_b))
...
class scholar():
    ...
    def find_studies_url_and_open_browser(ris_file, chunk_size):
        titles = utils.list_titles_from_ris(ris_file)

        # adjsuting the log format to inclide thread name with studiy title
        ...
        for chunk_of_titles in [titles[offs:offs+chunk_size] for offs in range(0, len(titles), chunk_size)]:
            input(f"\n\nPress Enter to proccess a chunk of ({len(chunk_of_titles)}) studies\n\n")
            # log.debug(f'processing a chunk of studies, size {len(chunk_of_titles)}')

            with Pool(chunk_size) as scholar_pool:
                scholar_pool.map(scholar.scholar_thread, chunk_of_titles)
    ...
    def scholar_thread(study_title):
        ...
        title_url = scholar.find_study_url_on_google_scholar(study_title)

        if title_url:
            log.info(f'opening browser, found [{title_url}] for {study_title}')
            utils.open_browser(title_url)
        else:
...
if __name__ == "__main__":
    ...
    if args.script == "ris":
        [log.info(title) for title in utils.list_titles_from_ris(args.ris_file)]
